...
class MyConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        if use_wscale:
            ...
            self.w_mul = lrmul
        self.weight = torch.nn.Parameter(
            torch.randn(output_channels, input_channels, kernel_size,
        ...
        layers = [('pixel_norm', PixelNormLayer()),
                  ('dense0',
                   MyLinear(512,
                            ...
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense0_act', act),
                  ('dense1',
                   MyLinear(512,
                            512,
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense1_act', act),
                  ('dense2',
                   MyLinear(512,
                            512,
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense2_act', act),
                  ('dense3',
                   MyLinear(512,
                            512,
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense3_act', act),
                  ('dense4',
                   MyLinear(512,
                            512,
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense4_act', act),
                  ('dense5',
                   MyLinear(512,
                            512,
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense5_act', act),
                  ('dense6',
                   MyLinear(512,
                            512,
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense6_act', act),
                  ('dense7',
                   MyLinear(512,
                            512,
                            gain=gain,
                            lrmul=0.01,
                            use_wscale=use_wscale)), ('dense7_act', act)]
...
class Truncation(nn.Module):
    def __init__(self, avg_latent, max_layer=8, threshold=0.7):
        ...
        )  # parameter of the module which is not trainable and is not passed to the optimizer when calling parameters() function
...
class InputBlock(nn.Module):
    ...
    def forward(self, dlatents_in_range):
        ...
        if self.const_input_layer:
            ...
            x = self.dense(dlatents_in_range[:,
                                             0]).view(batch_size, self.nf, 4,
                                                      4)
...
class GSynthesisBlock(nn.Module):
    def __init__(self, in_channels, out_channels, blur_filter, dlatent_size,
        ...
        if blur_filter:
            ...
            blur = None
        self.conv0_up = MyConv2d(in_channels,
                                 out_channels,
                                 kernel_size=3,
                                 gain=gain,
                                 use_wscale=use_wscale,
        ...
        self.epi1 = LayerEpilogue(out_channels, dlatent_size, use_wscale,
                                  ...
                                  use_styles, activation_layer)
        self.conv1 = MyConv2d(out_channels,
                              out_channels,
                              kernel_size=3,
                              gain=gain,
                              use_wscale=use_wscale)
                ...
                blocks.append(
                    (name,
                     InputBlock(channels, dlatent_size, const_input_layer,
                                gain, use_wscale, use_noise, use_pixel_norm,
                                use_instance_norm, use_styles, act)))
...
g_all.load_state_dict(torch.load('./server/models/sg1.pt'))
